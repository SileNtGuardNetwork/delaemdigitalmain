...
## 19. Edge Security / DNS Layer

Cloudflare добавляется как слой входной защиты и управления трафиком перед Vercel, Supabase и Timeweb. Это не хостинг и не backend, а броня на входе.
...
- Security Headers
- Redirect Rules
- Workers — вспомогательный edge-слой, не основной backend
- R2 — object storage при необходимости, не вместо Supabase/Timeweb на старте
- Zero Trust
- Access Policies
- Page Rules / Rulesets
- Origin protection
- Fallback DNS/provider strategy для РФ-critical проектов

Роли:
- Cloudflare — DNS, WAF, Turnstile, rate limiting, антибот, Zero Trust и edge-защита.
- Vercel — frontend, preview и production для Next.js.
- Supabase — data/backend/auth/storage/RAG.
- Timeweb Cloud — VDS, Docker, Telegram-боты, n8n, self-hosted и российский инфраструктурный контур.

Практический стандарт:
- Использовать Cloudflare DNS для доменов и поддоменов.
- SSL держать в режиме Full / Full strict, не Flexible.
- Ставить Turnstile на формы, квизы, регистрации и публичные submit endpoints.
- Ставить rate limit на формы, AI endpoints, login и публичные API.
- Использовать WAF и security headers как часть Cloud Care.
- Использовать Zero Trust для внутренних админок, staging, n8n и dashboards.
- Не включать агрессивный bot fight/proxy на Telegram webhooks без отдельных bypass/allow rules.
- Для РФ-only проектов проверять доступность и иметь fallback через Timeweb/Yandex/VK/Selectel/DDoS-Guard/Qrator/StormWall при необходимости.

## 20. Безопасность, мониторинг, DevOps
...
- Cloudflare DNS / WAF / CDN / Zero Trust
...
## 21. QA / тестирование / production quality
...
## 22. Юридический и операционный слой
...
- cloudflare-edge-template
- quality-gate-template
- client-handoff-template

## 24. Продуктовая линейка
...
- ClientFlow Edge Security — DNS, WAF, Turnstile, rate limiting, Zero Trust и защита входа.
...
5. Строим систему: стратегия → дизайн → код → AI → интеграции → аналитика → трафик → edge-security → сопровождение.
...
## 27. Финальная формула стека

GPT Business + Claude + Cursor Pro+ + Codex + GitHub + Vercel + Supabase + Timeweb Cloud + Cloudflare + Next.js + TypeScript + Tailwind + shadcn/ui + OpenAI/Anthropic/OpenRouter + Timeweb AI Agents + Telegram Bot API + n8n + CRM integrations + Яндекс.Метрика + PostHog + Sentry + Яндекс Директ + VK Ads + Telegram Ads + Avito + SEO + платежи + edge-security + мониторинг + QA + юридический слой.

Это закрывает фабрику на 100%: прототипы, копирайтинг, дизайн, верстку, адаптив, backend, интеграции, боты, AI, трафик, аналитику, платежи, edge-security, сопровождение и production-контроль.
